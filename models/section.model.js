...
Section.findById = function (id, result) {
    connection.query(`SELECT * FROM section WHERE idSection = ${id}`, (err, res) => {
        if (err) {
...
Section.findBySlug = function (id, result) {
    connection.query(`SELECT * FROM section WHERE sectionSlug = '${id}'`, (err, res) => {
        if (err) {
...
Section.update = function (id, data, result) {
    connection.query(`UPDATE section SET ? WHERE idSection = ${id}`, data, (err, res) => {
        if (err) {
...
Section.delete = function (id, result) {
    connection.query(`DELETE FROM section WHERE idSection = ?`, id, (err, res) => {
        if (err) {
            console.log("Error while deleting section by ID", err);
            result(err, null);
            return;
        }
        result(null, {message: `Section ID ${id} has been deleted successfully`});
    })
}
