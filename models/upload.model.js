...
Upload.create = function (uploadInfo, result) {
  var files = uploadInfo.files.map((e) => [
    "uploads/view/" + e.filename,
    e.mimetype,
    uploadInfo.createdBy,
    uploadInfo.sectionId,
    e.originalname,
  ]);
  connection.query(
    `INSERT INTO upload (path, mime, createdBy, sectionId, filename) VALUES ?`,
    [files],
    (err, res) => {
      if (err) {
...
Upload.getAll = function (result) {
  connection.query(
    `SELECT * FROM upload ORDER BY idUpload DESC`,
    (err, res) => {
      if (err) {
...
Upload.findById = function (id, result) {
  connection.query(
    `SELECT * FROM upload WHERE idUpload = ${id}`,
    (err, res) => {
      if (err) {
...
Upload.findBySectionId = function (id, result) {
  connection.query(
    `SELECT * FROM upload WHERE sectionId = ${id} ORDER BY idUpload DESC`,
    (err, res) => {
      if (err) {
        console.log("Error while getting file by ID", err);
        result(err, null);
...
Upload.update = function (id, data, result) {
  connection.query(
    `UPDATE upload SET ? WHERE idUpload = ${id}`,
    data,
    (err, res) => {
      if (err) {
...
Upload.delete = function (id, result) {
  connection.query(
    ...
    (checkErr, checkRslt) => {
      ...
      connection.query(
        `DELETE FROM upload WHERE idUpload = ?`,
        id,
        (err, res) => {
          if (err) {
          ...
          try {
            ...
            fs.unlinkSync(
              path.join(__dirname, "..", "uploads/" + finalPath[2]),
            );
            result(null, {
