...
Student.getAll = function (result) {
    connection.query(`SELECT student.*, studySection.sectionName FROM student JOIN studySection ON student.studySectionId = studySection.idSection LEFT JOIN blockReason ON student.blockReasonId = blockReason.idBlockReason`, (err, res) => {
        if (err) {
