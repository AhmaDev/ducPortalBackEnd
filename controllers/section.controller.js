...
exports.createFee = (req, res) => {
  Section.createFee(req.body, (err, data) => {
    if (err) res.sendStatus(500);
    else res.send(data);
  });
};
...
exports.findAllFees = (req, res) => {
  Section.findAllFees((err, data) => {
    if (err) {
      if (err.kind == "not_found") res.sendStatus(404);
      else res.sendStatus(500);
    } else res.send(data);
...
exports.findFeesBySection = (req, res) => {
  Section.findFeesBySection(req.params.id, (err, data) => {
    if (err) {
      if (err.kind == "not_found") res.sendStatus(404);
      else res.sendStatus(500);
    } else res.send(data);
